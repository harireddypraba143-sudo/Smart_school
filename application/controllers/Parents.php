...
class Parents extends CI_Controller { 
    ...
    /************** FEATURE 4: PARENT CHILD MARKS VIEW ********************/
    function child_marks($exam_id = '') {
        if ($this->session->userdata('parent_login') != 1) redirect(base_url(), 'refresh');
        
        $parent_id = $this->session->userdata('parent_id');
        $child = $this->db->get_where('student', ['parent_id' => $parent_id])->row();

        $page_data['student'] = $child;
        $page_data['student_id'] = $child ? $child->student_id : 0;
        $page_data['exam_id'] = $exam_id;
        $page_data['page_name'] = 'child_marks';
        $page_data['page_title'] = 'Child Report Card';
        $this->load->view('backend/index', $page_data);
    }
