...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('accountant_login')== 1) redirect (base_url(). 'admin/dashboard');
        if ($this->session->userdata('admission_login')== 1) redirect (base_url(). 'admin/dashboard');
  ...
  //********************************** the function below validating user login request 
    ...
    function logout(){
      ...
      if($login_user == 'admission'){
        $this->login_model->logout_model_for_admission();
      }
      if($login_user == 'librarian'){
